...
def run_simulation(config):
    ...
    pygame.display.set_caption("Senaryo 2: Sıralı Adaptif Trafik Işığı")
    ...
    GREEN_LIGHT_ORDER = ["yukari", "sag", "asagi", "sol"]
    ...
    class TrafikIsigiSiraliAdaptif:
        def __init__(self):
        ...
        def _calculate_adaptive_green_frames(self, yon, kuyruklar_dict):
            queue_len = len(kuyruklar_dict[yon])
        ...
        def guncelle(self, kuyruklar_dict_ref):
            ...
            if self.current_light_state == "INITIALIZING":
                ...
                for yon in self.durumlar:
                    self.durumlar[yon] = "kirmizi"

            elif self.current_light_state == "INTERGREEN_ALL_RED":
                if self.timer_frames <= 0:
                    self.current_light_state = "GREEN"
                    self.timer_frames = self._calculate_adaptive_green_frames(self.active_direction, kuyruklar_dict_ref)
    ...
    trafik_yonetimi = TrafikIsigiSiraliAdaptif()
